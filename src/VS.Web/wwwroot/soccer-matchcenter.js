...
let visibleTimelineEvents=20;
const periodName=value=>{const key=String(value||'match').toLowerCase().replace(/[^a-z]/g,'');return({firsthalf:'First Half',secondhalf:'Second Half',extratimefirsthalf:'Extra Time · First Half',extratimesecondhalf:'Extra Time · Second Half',penaltyshootout:'Penalty Shootout',prematch:'Pre-Match',postmatch:'Full Time'})[key]||String(value||'Match').replace(/([a-z])([A-Z])/g,'$1 $2');};
function timelineHtml(events){let lastPeriod=null;return events.map(event=>{const divider=event.period!==lastPeriod?`<div class="timeline-period">${esc(periodName(event.period))}</div>`:'';lastPeriod=event.period;return `${divider}<div class="pitch-item"><strong>${esc(event.minute)}' · ${esc(event.description)}</strong><span class="muted">${esc(event.teamName)}</span></div>`;}).join('');}
...
async function load() {
  ...
  const timeline=document.querySelector('#timeline');timeline.style.setProperty('--timeline-visible-events',visibleTimelineEvents);setHtmlIfChanged(timeline,timelineHtml(matchCenter.events));
...
Promise.all([fetch('/api/soccer/match-condition-settings',{cache:'no-store'}).then(response=>response.json()),fetch('/api/soccer/matchcenter-display-settings',{cache:'no-store'}).then(response=>response.json())]).then(([conditions,display])=>{conditionSelection={...conditionSelection,...conditions};visibleTimelineEvents=display.visibleTimelineEvents||20;return load();}).catch(()=>load().catch(showError));
